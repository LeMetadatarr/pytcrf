...
The `pytcrf` top-level package re-exports everything below.

## Lookup functions

### `get_game(title, *, with_wikitext=False, with_section_text=False, platforms=None, transport=None) -> GamePage | None`

Fetch one article's section structure through `action=parse`. Returns `None`
when the page does not exist.

- `with_wikitext`: also return the raw page wikitext (enables infobox
  platform detection). pytcrf fetches wikitext by default unless you supply
  `platforms` for a heavy section-text crawl.
- `with_section_text`: fetch and attach plain text to every `Section` (one
  extra API call per section).
- `platforms`: override the platform list (otherwise inferred from the
  infobox).
...
The platform names under `Category:Games by platform` (for example `Genesis`,
`NES`).

### `iter_games(*, platforms=None, per_platform_limit=None, transport=None) -> Iterator[GamePage]`

Walk the `Category:Games` tree and yield section-less `GamePage` stubs with
`title`, `pageid`, and detected `platforms`. Call `get_game` on a stub to get
its structure. A page found under several platforms is yielded once, with
all its platforms merged.
...
A generic `categorymembers` walk, with transparent continuation.
`member_type` is `"page"`, `"subcat"`, or `"file"`.
...
Properties: `is_notable` (flags a heading as cut/unused/regional/debug
content). Methods: `to_dict()`, `from_api(node)`.
...
`pytcrf.dataset`: see [dataset.md](dataset.md): `iter_page_rows`,
`page_to_rows`, `export_jsonl`, `build_pages_dataset`.

---
[← Quickstart](quickstart.md) · [Home](../README.md) · [The Category:Games tree →](categories.md)
